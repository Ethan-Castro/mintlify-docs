...
> This research was supported, in part, under National Science Foundation Grants CNS-0958379, CNS-0855217, ACI-1126113, and OAC-2215760, and the City University of New York High Performance Computing Center at the College of Staten Island.
...
<Warning>
  **Jobs are never to be run on the head (login) node.** Any job found running there will be killed, and the owning account may be suspended. Log in, edit files, submit SLURM jobs — that's what the login node is for. Anything heavier runs through `sbatch` or `srun`.
</Warning>
...
Data on suspended accounts is not retained indefinitely — copy out anything you want to keep before you leave.
...
This documentation aims to meet **WCAG 2.1 AA**. If you encounter a barrier — screen-reader trouble, color-contrast issues, missing alt text, keyboard navigation problems — let us know so it can be fixed.
